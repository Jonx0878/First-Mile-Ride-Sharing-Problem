...
public:
    ...
    void create_model(bool relax = true, bool silent = true) {
        ...
        for (const int& j : customers) {
            int ub = std::max(int(max_cap), demand[j]);
            for (const int& k : vehicles) {
                int idx = k * no_cust_d + j + 1;
                if (var_indices_subset.find(idx) != var_indices_subset.end()) {
                    lexpr += veh_occ[k] * vars[idx];
                    lexpr2 += (demand[j] + veh_occ[k] - ub) * vars[idx];
                }
            }
            model->addConstr(phi[j] >= demand[j] + lexpr);
            model->addConstr(phi[j] <= ub + lexpr2);
            lexpr.clear();
            lexpr2.clear();
        }
        model->update();
        ...
        for (const int& i : customers) {
            for (const int& j : customers) {
                if (i == j) continue;
                int ub = std::max(std::max(demand[j], demand[i]), int(max_cap)) - std::min(demand[i], demand[j]);
                for (const int& k : vehicles) {
                    int idx = no_veh * no_cust_d + k * (pow(no_cust_d, 2) - no_cust - 1) + (i - 1) * no_cust_d + j + 1;
                    if (var_indices_subset.find(idx) != var_indices_subset.end()) {
                        lexpr += (ub + demand[j]) * vars[idx];
                        //lexpr2 += vars[idx];
                    }

                    idx = no_veh * no_cust_d + k * (pow(no_cust_d, 2) - no_cust - 1) + (j - 1) * no_cust_d + i + 1;
                    if (var_indices_subset.find(idx) != var_indices_subset.end()) {
                        lexpr += (ub - demand[i]) * vars[idx];
                        //lexpr2 += vars[idx];
                    }
                }

                model->addConstr(phi[i] - phi[j] + lexpr <= ub);

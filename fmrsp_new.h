...
private:
    ...
    void construct_variables(const char& vtype) {
        ...
        for (const int& idx : chi_indices) {
            ...
            const int obj = cust_dist[i][j] - demand[i] * cust_dist[i][0]; // Change to penalty
        ...
        for (const int& idx : psi_indices) {
            ...
            if (main_type == "2I") obj += - demand[i] * cust_dist[i][0];
        ...
        for (const int& idx : rho_indices) {
            ...
            rho[idx] = model->addVar(0.0, std::min(cust_arr_time[i], cust_arr_time[j]) - cust_dist[i][j] - min_dist, 0, GRB_CONTINUOUS, name);
        }
        ...
        for (const int& idx : rho_veh_indices) {
            ...
            rho_veh[idx] = model->addVar(0.0, std::min(veh_arr_time[k], cust_arr_time[j]), 0, GRB_CONTINUOUS, name);
        }
    ...
    void add_constraints() {
        ...
        if (time_type == "TIME") {
            ...
            for (const int& i : customers) {
                const int rho_idx = i * no_cust_d + 0;
                if (rho_indices.find(rho_idx) == rho_indices.end()) continue;
                if (main_type == "2I") {
                    const int idx = i * no_cust_d + 0;
                    if (psi_indices.find(idx) != psi_indices.end()) {
                        lexpr2 += (cust_arr_time[i] - cust_dist[i][0]) * psi[idx];
                    }
                }
                else {
                    for (const int& k : vehicles) {
                        const int idx = k * no_cust_d_squared + i * no_cust_d + 0;
                        if (chi_indices.find(idx) != chi_indices.end()) {
                            lexpr2 += (cust_arr_time[i] - cust_dist[i][0]) * chi[idx];
                        }
                    }
                }
                model->addConstr(rho[rho_idx] <= lexpr2);

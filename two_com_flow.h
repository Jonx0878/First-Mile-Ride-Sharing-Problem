...
public:
    ...
    void create_model(bool relax = true, bool silent = true) {
        ...
        for (const int& idx : flow_indices) {
            ...
            flow[idx] = model->addVar(0, ub, 0, GRB_CONTINUOUS, name);
        }
...
// Edge from vehicle if chosen
        ...
        for (const int& i : customers) {
            for (const int& k : vehicles) {
                ...
                if (var_indices_subset.find(idx) != var_indices_subset.end()) lexpr2 += (capacity - veh_occ[k]) * vars[idx];
            }
            for (const int& j : dest_and_cust) {
                if (i == j) continue;
                int idx = j * no_cust_d + i;
                if (j > 0 && flow_indices.find(idx) != flow_indices.end()) lexpr3 += flow[idx];
                idx = i * no_cust_d + j;
                if (flow_indices.find(idx) != flow_indices.end()) lexpr4 += flow[idx];
            }
            for (const int& k : vehicles) {
                int idx = k * no_cust_d + i;
                if (var_indices.find(idx) != var_indices.end()) lexpr4 += 2 * demand[i] * xi[i][k];
            }
